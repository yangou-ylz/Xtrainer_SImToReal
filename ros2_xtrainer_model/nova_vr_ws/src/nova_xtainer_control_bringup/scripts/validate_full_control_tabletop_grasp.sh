...
SESSION="${1:-session_full_control_tabletop_grasp}"
GUI="${2:-false}"
...
xacro src/nova_xtainer_control_description/urdf/generated/xtrainer_full_control.urdf.xacro hardware_plugin:=gazebo_ros2_control/GazeboSystem > /tmp/xtrainer_full_control_tabletop_validate.urdf
check_urdf /tmp/xtrainer_full_control_tabletop_validate.urdf >"${CAPTURE_DIR}/check_urdf.txt"
...
ros2 run nova_xtainer_control_bringup xtrainer_full_tabletop_grasp.py \
  --session-name "${SESSION}" \
  --object-name xtrainer_grasp_block \
  --support-name xtrainer_grasp_support \
  "${@:3}" \
  >"${CAPTURE_DIR}/tabletop_grasp.log" 2>&1

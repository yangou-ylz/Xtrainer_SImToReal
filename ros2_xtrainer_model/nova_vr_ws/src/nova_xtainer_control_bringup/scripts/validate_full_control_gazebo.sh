...
SESSION="${1:-session_full_control_gazebo}"
GUI="${2:-false}"
...
xacro src/nova_xtainer_control_description/urdf/generated/xtrainer_full_control.urdf.xacro hardware_plugin:=gazebo_ros2_control/GazeboSystem > /tmp/xtrainer_full_control_gazebo_validate.urdf
check_urdf /tmp/xtrainer_full_control_gazebo_validate.urdf >"${CAPTURE_DIR}/check_urdf.txt"
...
ros2 run nova_xtainer_control_bringup xtrainer_full_command14.py \
  ...
  --duration 0.8 \
  --repeat-sec 1.0 \
  --command14 0.10 -0.10 -0.80 0.20 -1.00 0.10 0.20 -0.10 -0.10 0.80 -0.20 1.00 -0.10 0.20 \
  >"${CAPTURE_DIR}/command.log" 2>&1
sleep 3
